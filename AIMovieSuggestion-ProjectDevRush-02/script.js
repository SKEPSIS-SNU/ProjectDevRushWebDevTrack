document.addEventListener('DOMContentLoaded', () => {
    ...
    const nextRecommendation = async (media) => {
        ...
        try {
            ...
            loadFeaturedContent(recomendation);
    ...
    async function loadFeaturedContent(recomendation) {
        ...
        //     'tt0773262'  // Dexter
        // ];
        const featuredImdbIDs = recomendation;
        ...
        for (const imdbID of featuredImdbIDs) {
            ...
            try {
                const response = await fetch('http://localhost:8000/aia', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
                    body: JSON.stringify({ currently_watching: movie })
                });
            } catch (error) {
                console.error('Error fetching recommendation:', error);
            }
